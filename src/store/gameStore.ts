...
export interface ShuffledDeckOrder {
...
function orderMainDeckCardsByNames(
  cards: CardData[],
  orderedNames: string[]
) {
  ...
  for (const card of cards) {
    const current = buckets.get(card.name) ?? [];

    current.push(card);
    buckets.set(card.name, current);
  }

  const orderedCards: CardData[] = [];
  ...
  for (const name of orderedNames) {
    ...
    orderedCards.push(bucket.shift() as CardData);
  }

  return orderedCards;
}

export function createShuffledDeckOrder(
  player1Deck: CardData[],
  player2Deck: CardData[]
): ShuffledDeckOrder {
  return {
    player1MainDeckNames: shuffle(getMainDeckCards(player1Deck)).map(
      (card) => card.name
    ),
    player2MainDeckNames: shuffle(getMainDeckCards(player2Deck)).map(
      (card) => card.name
    ),
  };
}
...
function createPlayer(
  deck: CardData[],
  mainDeckNameOrder?: string[]
): PlayerState {
  const leader =
    deck.find((x) => x.type === "leader") ||
    null;

  const mainDeckCards = getMainDeckCards(deck);

  const mainDeck = mainDeckNameOrder
    ? orderMainDeckCardsByNames(
      mainDeckCards,
      mainDeckNameOrder
    )
    : shuffle(mainDeckCards);
