...
export default function DeckSelect() {
  ...
  const startGameWithDeckOrder = useGameStore(
    (x) => x.startGameWithDeckOrder
  );
  ...
  useEffect(() => {
    return onShuffleOrderReceived((deckOrder) => {
      if (isHost) {
        return;
      }
